function [emed, cv, mag, susc,dmed]=SMC_Ising1d(T,H,N,npassos, nequi)
% unidade de energia uE=J; uT=J/kB
%s=ones(1,N); % começamos do estado com si=1 para todo o i
...
M=0;
for i=1:N
    E=E-s(i)*s(listav(i,2))-H*s(i);
    M=M+s(i);
end
emed=0; cv=0; mag=0; susc=0; dmed=0;
...
for t=1:npassos
    ...
    if t> nequi
        emed=emed+E; cv=cv+E^2;  mag=mag+M; susc=susc+M^2;
        dmed=dmed+dominios(s,listav,N);
    end
end
nmedidas=npassos-nequi;
emed=emed/nmedidas; cv=cv/nmedidas; mag=mag/nmedidas; susc=susc/nmedidas;
cv=(cv-emed^2) / T^2; susc=(susc-mag^2)/T;
dmed=dmed/nmedidas;
...
for i=1:N
    listav(i,1)=i-1; listav(i,2)=i+1; nv(i)=2;
    if i==1
        listav(i,1)=N;
    end
    if i==N
