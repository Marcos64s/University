...
fprintf(1,'Tamanho da maior componente=%d Label da maior componente=%d\n',S,lmax)
kmax=max(nv);
fprintf(1,'kmax=%d\n',kmax)
tmax=10000;

g=1/kmax; % taxa de recuperação
...
xi=zeros(tmax,1); xs=zeros(tmax,1);
xi(1)=1/N; xs(1)=1-1/N;
Ni=1; Ns=N-1; Nr=0;
xinovo=xi(1);
for t=2:tmax
    tv(t)=tv(t-1)+dt*N;
        for act=1:N
            ...
            elseif(s(i)==1) % recuperação de um infetado
                if rand(1) < 1/kmax
                    s(i)=2;
                     Ni=Ni-1; Nr=Nr+1;
                end
