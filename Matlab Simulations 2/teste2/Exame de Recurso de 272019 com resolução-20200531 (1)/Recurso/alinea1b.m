...
for  T=Tv
 ...
 [Emedio(i), E2medio(i), nkmedio]=metropolisFermioes(T,nequi,nmedidas,N,nmax);
 ...
 Emedio(i)=Emedio(i)-2*N; % subtrai a energia do estado de menor energia de cada particula.
 %calculo do valor medido de z
 zmedio(i)=1/((1-nkmedio(1))/nkmedio(1));
 
 %espressoes teoricas esperadas
 zteorv(i)=exp(4*N/Tv(i)/pi)-1; 
 Eteorv(i)=-(pi/4)*Tv(i)*Tv(i)*dilog(zteorv(i)+1);
...
ylabel('<E>')  
figure(3)
loglog(Tv,zteorv,'k-',Tv,zmedio,'r.')
axis([0,Tv(end),0,1e4])
xlabel('T')
ylabel('z')   
