 ...
 *  \brief watcher waits for ingredient generated by agent
 *
 *  Watcher updates state and waits for ingredient from agent, then 
 *  checks agent is closing.
 *  If agent is closing, watcher should update state again and inform
 *  the smoker that holds the ingredient of the watcher so that it can terminate.
 ...
 *  \brief watcher updates reservations in shared mem and checks if some smoker can complete a cigarette
 *
 *  Watcher updates state and reserves ingredient and then checks if 
 *  some smoker may start rolling a cigarette.
...
if((sh->fSt.reserved[PAPER] == 1) && (sh->fSt.reserved[MATCHES] == 1)){
    ret = TOBACCO;
    sh->fSt.reserved[PAPER]--;
    sh->fSt.reserved[MATCHES]--;
}else if((sh->fSt.reserved[PAPER] == 1) && (sh->fSt.reserved[TOBACCO] == 1)){
    ret = MATCHES;
    sh->fSt.reserved[PAPER]--;
    sh->fSt.reserved[TOBACCO]--;
}else if((sh->fSt.reserved[MATCHES] == 1) && (sh->fSt.reserved[TOBACCO] == 1)){
    ret = PAPER;
    sh->fSt.reserved[MATCHES]--;
    sh->fSt.reserved[TOBACCO]--;
}
