 ...
 *
 *  smoker updates state and waits for watcher notification to proceed to 
 *	roll cigarette.
 *  After the notification, smoker should update the inventory of ingredients.
 *  It may also happen that watcher will notify smoker not because ingredients 
 *	are available but because the factory is closing. In this case, state should 
 *	be updated and the function should return false;  
 *
 *  \param id smoker id, that is related to the ingredient that the smoker holds
 *	(see HAVE* constants in probConst.h)
 *
...
if(semDown(semgid,sh->wait2Ings[id]) == -1) {                                                  /* enter critical region */
perror ("error on the down operation for semaphore access (CT)");
exit (EXIT_FAILURE);
}
...
static void rollingCigarette (int id)
{
	double rollingTime = 100.0 + normalRand(3.0);

    if (semDown (semgid, sh->mutex) == -1)  {                                                     /* enter critical region */
	perror ("error on the up operation for semaphore access (SM)");
	exit (EXIT_FAILURE);
}
    /*Semafro para que de update*/
    sh->fSt.st.smokerStat[id] = ROLLING; // 
    saveState(nFic, &sh->fSt);

    usleep(rollingTime);    
    /*Meter aqui um signal qualquer*/
