...
saveState(nFic,&sh->fSt);
    ...
    /* Manufacturing 2 aleatory ingridients and updating to inventory*/
int ing1=rand()%NUMINGREDIENTS;
int ing2=rand()%NUMINGREDIENTS;
while(ing1==ing2)
{
...
if (semUp(semgid,sh->ingredient[ing1]) == -1) {                                                      /* enter critical region */
perror ("error on the up operation for semaphore access (AG)");
exit (EXIT_FAILURE);
}
if (semUp(semgid,sh->ingredient[ing2]) == -1) {                                                      /* enter critical region */
perror ("error on the up operation for semaphore access (AG)");
exit (EXIT_FAILURE);
}
...
static void waitForCigarette ()
{
    ...
    /*espera até ter "luz verde" ----> depois de enrolar o cigarro*/
    /*Updating Agent Stat*/
...
if (semUp(semgid,sh->ingredient[TOBACCO]) == -1) {                                                      /* enter critical region */
perror ("error on the up operation for semaphore access (AG)");
exit (EXIT_FAILURE);
}
if (semUp(semgid,sh->ingredient[PAPER]) == -1) {                                                      /* enter critical region */
perror ("error on the up operation for semaphore access (AG)");
exit (EXIT_FAILURE);
}
if (semUp(semgid,sh->ingredient[MATCHES]) == -1) {                                                      /* enter critical region */
perror ("error on the up operation for semaphore access (AG)");
exit (EXIT_FAILURE);
}

    if ((shmid = semUp (semgid, sh->mutex)) == -1) {                                                        /* leave critical region */
perror ("error on the up operation for semaphore access (AG)");
exit (EXIT_FAILURE);
}
